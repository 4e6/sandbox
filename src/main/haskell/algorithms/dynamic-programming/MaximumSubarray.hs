...
maxSubseq :: [Int] -> Int
maxSubseq xs = maxSubseq' xs 0 0 0 (minBound :: Int) (0, 0)

maxSubseq' :: [Int] -> Int -> Int -> Int -> Int -> (Int, Int) -> Int
maxSubseq' [] _ _ _ maxSum _ = maxSum
maxSubseq' (x:xs) curSrt curEnd curSum maxSum (maxSrt,maxEnd) =
  let curSum' = curSum + x
      curEnd' = curEnd + 1
      (maxSum',maxSrt',maxEnd') = if curSum' > maxSum then (curSum',curSrt,curEnd) else (maxSum,maxSrt,maxEnd)
      (curSumn', curSrt') = if curSum' < 0 then (0, curEnd+1) else (curSum',curSrt)
  in maxSubseq' xs curSrt' curEnd' curSumn' maxSum' (maxSrt', maxEnd')

maxNonSubseq :: [Int] -> Int
maxNonSubseq xs
  | all (<0) xs = maximum xs
